...
import { fetchActiveCustomer, deleteCustomer, fetchBlackListedCustomer, fetchActiveCustomerCount, fetchBlackListedCustomerCount, updateStatusActivate, updateStatusBacklist } from '../../../features/qcustomers/qcustomerSlice';
...
const QcustomerCard = ({ onSelect }) => {
    ...
    const handleAdd = () => navigate('/qcustomerform');
    const handleView = (customerId) => navigate(`/qcustomerview/${customerId}`);
    const handleEdit = (customerId) => {
        navigate(`/qcustomerupdate/${customerId}`)
    }
    ...
    return (
        <Box sx={{ p: 3 }}>
            <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                <Typography variant="h5" fontWeight={600}>Manage Quotation Customers</Typography>
                <Button
                    variant="contained"
                    color="primary"
                    startIcon={<AddIcon />}
